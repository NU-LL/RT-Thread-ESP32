...
{
    rt_uint32_t b = 3,c = 0;
    rt_kprintf("address of b is 0x%08x\n",&b);
    rt_kprintf("address of c is 0x%08x\n",&c);
}
...
//Enable trace memory and immediately start trace.
    ...
    // //这里做个小测试
    // struct rt_thread testthread = {0};
    // ESP_EARLY_LOGI(TAG,"sp offset: %d[0x%x] bit", ((void *)&testthread.sp - (void *)&testthread), ((void *)&testthread.sp - (void *)&testthread));// 0x1c
    // // testthread.sp = 0x12345678;
    // *(rt_uint32_t *)(((void *)(&testthread))+0x1c) = 0x12345678;
    // ESP_EARLY_LOGI(TAG,"tlist:[0x%x 0x%x] sp:[0x%x] entry:[0x%x]", testthread.tlist.prev, testthread.tlist.next, testthread.sp, testthread.entry);
